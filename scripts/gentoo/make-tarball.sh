...
for myname in 00*.patch ; do
	mv ${myname} tmp/patch/$(echo ${myname}|sed -e 's:^\(....\)-:\1_all_:') || exit 1
done

# add the extra patch

cp scripts/gentoo/0000_all_Gentoo-Git-is-development tmp/patch/0000_all_Gentoo-Git-is-development.patch || exit 1
cp scripts/gentoo/9999_all_Gentoo-We-make-a-release tmp/patch/9999_all_Gentoo-We-make-a-release.patch || exit 1
